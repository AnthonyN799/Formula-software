...
if check_password():
        ...
        with st.form("add_material_form", clear_on_submit=True):
            ...
            if submitted and new_trade != "":
                ...
                if tds_file is not None:
                    file_name = f"{new_rm_code}_TDS.pdf"
    ...
    # --- PAGE 2: FORMULA HUB ---
        ...
        edited_df = st.data_editor(
            ...
            use_container_width=True,
            column_config={
                "Ingredient": st.column_config.SelectboxColumn("Select Ingredient", options=ingredient_list, required=True),
                "Percentage (%)": st.column_config.NumberColumn("Percentage (%)", min_value=0.0, max_value=100.0, step=0.1, required=True)
            }
        )
        ...
        if st.button("Save Formula"):
            if formula_name == "" or total_percent != 100.0 or edited_df["Ingredient"].isnull().any():
                st.error("Please ensure the name is filled, ingredients are selected, and the total is exactly 100%.")
            else:
        ...
        # 2. CALCULATE & PRODUCE BATCH
        st.subheader("⚗️ Batch Production & Cost")
        ...
        if not formulas_df.empty:
            ...
            for ingredient, percentage in selected_recipe.items():
                ...
                if not match.empty:
                    ...
                    cost_breakdown.append({
                        ...
                        "Required (g)": round(amount_needed_g, 2), 
                        "Required (Kg)": round(amount_needed_g / 1000, 4),
                        "Cost ($)": f"${ing_cost:.2f}"
                    })
                else:
                    st.error(f"⚠️ Missing live price data for: {ingredient}.")
            ...
            if cost_breakdown:
                ...
                if st.button("🚀 Produce Batch & Deduct Stock"):
                    stock_issues = []
                    updates = []
                    ...
                    for item in cost_breakdown:
                        ...
                        amount_needed_kg = item["Required (Kg)"]
                        
                        current_stock = float(inventory[inventory['trade_name'] == ingredient]['quantity_kg'].values[0])
                        
                        if current_stock < amount_needed_kg:
                            stock_issues.append(f"{ingredient} (Need {amount_needed_kg} Kg, only have {current_stock} Kg)")
                        else:
                            updates.append({
                                "trade_name": ingredient,
                                "new_stock": current_stock - amount_needed_kg
                            })
                            
                    if stock_issues:
                        st.error("⚠️ Cannot produce batch. Insufficient stock for:")
                        for issue in stock_issues:
                            st.write(f"- {issue}")
                    else:
                        # Safety check passed, execute the deduction in Supabase!
                        for u in updates:
                            supabase.table('inventory').update({'quantity_kg': u['new_stock']}).eq('trade_name', u['trade_name']).execute()
                        
                        st.balloons()
                        st.success(f"✅ Successfully produced {batch_size_g}g of {selected_formula_name}!")
                        st.info("Inventory quantities have been successfully deducted from the vault.")
                        st.rerun()
        else:
