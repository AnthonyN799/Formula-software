...
if check_password():
    ...
    if menu == "Raw Material Library":
        ...
        st.dataframe(display_inv[['rm_code', 'trade_name', 'inci_name', 'price_per_kg', 'Cost/gram ($)', 'quantity_kg', 'function']], use_container_width=True, hide_index=True)
        ...
        with col_del:
            ...
            if not inventory.empty:
                to_delete = st.selectbox("Material to Remove", inventory['trade_name'].tolist(), key="del_mat")
                confirm = st.checkbox("I want to delete this material", key="conf_mat")
                pass_check = st.text_input("Security Passcode", type="password", key="pass_mat")
                if st.button("Permanently Delete", type="primary", disabled=not (confirm and pass_check == "lab2026")):
                    supabase.table('inventory').delete().eq('trade_name', to_delete).execute()
    ...
    # --- PAGE 2: FORMULA HUB ---
        ...
        col_new, col_del_f = st.columns([2, 1])
        with col_new:
        ...
        with col_del_f:
            if not formulas_df.empty:
                with st.expander("🗑️ Delete Formula"):
                    to_del_f = st.selectbox("Formula to Remove", formulas_df['formula_name'].tolist())
                    confirm_f = st.checkbox("I confirm deletion", key="conf_f")
                    pass_f = st.text_input("Enter Passcode", type="password", key="p_f")
                    if st.button("Wipe Formula", type="primary", disabled=not (confirm_f and pass_f == "lab2026")):
                        supabase.table('formulas').delete().eq('formula_name', to_del_f).execute()
                        st.rerun()
        ...
        if not formulas_df.empty:
            ...
            with c1:
                if st.button("🔍 Check Stock", use_container_width=True):
                    if stock_ok: st.success("Sufficient Stock")
    ...
    # --- PAGE 3: PRODUCTION LOGS ---
        ...
        if logs.data:
            ...
            st.divider()
            with st.expander("🛡️ Advanced: Wipe Record"):
                st.warning("This will permanently remove a manufacturing record from history.")
                log_to_wipe = st.selectbox("Select Lot Number to Wipe", df['lot_number'].tolist())
                c_wipe = st.checkbox("I understand this is permanent", key="c_w")
                p_wipe = st.text_input("Admin Passcode", type="password", key="p_w")
                if st.button("Wipe from History", type="primary", disabled=not (c_wipe and p_wipe == "lab2026")):
                    supabase.table('production_records').delete().eq('lot_number', log_to_wipe).execute()
                    st.rerun()
